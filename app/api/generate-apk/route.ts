...
/* --------------------- 轻量日志工具 --------------------- */
function safeJson(x: any, lim = 2000) {
  try {
...
function log(kind: "debug" | "info" | "warn" | "error", ctx: Record<string, any>, msg: string, extra?: any) {
  const line = `[NDJC][${kind.toUpperCase()}] step=${ctx.step || "-"} runId=${ctx.runId || "-"} ${msg}`;
  // eslint-disable-next-line no-console
  (console as any)[kind] ? (console as any)[kind](line, extra ?? "") : console.log(line, extra ?? "");
}
...
async function assertCoreArtifactsOrExplain(runId: string) {
  ...
  const miss = [!has02 ? "02_plan.json" : null, !has03 ? "03_apply_result.json" : null].filter(Boolean).join(", ");
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    log("info", { step, runId }, `incoming request body=${safeJson(input, 1200)}`);
    await jWriteJSON(runId, "00_input.json", input);

    step = "fetch-templates";
    const tplFetch = await ensureLatestTemplates(runId);
    log("info", { step, runId }, `templates source=${safeJson(tplFetch)}`);
    ...
    const tplDirCandidates = [path.join(tplRoot, `${templateName}`), path.join(tplRoot, `${templateName}-template`)];
    ...
    log("debug", { step, runId }, "path/env checks done", checks);
    ...
    try {
      ...
      log("info", { step, runId }, `orchestrate online with ${groqModel}`);
      ...
      log("warn", { step, runId }, "orchestrate fallback to offline A-mode", err?.message);
    }
    ...
    if (wantContractV1) {
      ...
      if (!validation.ok) {
        log("warn", { step, runId }, "contract v1 validation failed", validation.issues);
      ...
      log("info", { step, runId }, "contract v1 mapped to plan");
    }
    ...
    log("info", { step, runId }, "plan generated");
    ...
    log("info", { step, runId }, `materialized to ${appRoot}`);
    ...
    log("info", { step, runId }, `critical replacements=${replacedTotal}`);
    ...
    if (o.mode === "B" && o.allowCompanions && Array.isArray(o.companions) && o.companions.length) {
      ...
      log("info", { step, runId }, `companions emitted=${emitted.written}`);
    } else {
    ...
    const anchors =
      (applyResult || [])
        .flatMap((r: any) => (r?.changes || []).map((c: any) => `- \`${c.marker}\` @ \`${r.file}\` → replaced=${c.replacedCount}, found=${c.found}`))
        .join("\n") || "- (no markers found)";
...
`;
    ...
    if (process.env.NDJC_GIT_COMMIT === "1") {
      ...
      log("info", { step, runId }, "git committed", commitInfo);
    } else {
    ...
    if (!okCore) {
      await pushDirByContentsApi(
        reqLocalDir,
        `requests/${runId}`,
        runBranch,
        `[NDJC ${runId}] logs (pre-dispatch error)`
      );
    ...
    if (process.env.NDJC_SKIP_ACTIONS === "1" || (input as any)?.skipActions === true) {
      ...
      log("info", { step, runId }, "actions skipped");
    } else {
      const inputs = {
        runId,
        branch: runBranch,
        ...
        preflight_mode: (input as any)?.preflight_mode || "warn",
      };
      ...
      log("info", { step, runId }, `actions dispatched (degraded=${dispatch.degraded}) url=${actionsUrl}`);
    }
    ...
    // 关键：把错误完整打印到终端，便于本地排查 500
    log("error", { step, runId }, `request failed: ${String(e?.message ?? e)}`, e?.stack || e);
    return NextResponse.json(
