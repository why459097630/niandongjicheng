...
async function emitCompanions(
  ...
  for (const file of companions) {
    const rel = file.path.replace(/^[/\\]+/, ''); // 规范化
...
function normalizeWorkflowId(wf: string) {
  if (/^\d+$/.test(wf)) return wf; // 数字 id
  if (wf.endsWith('.yml')) return wf; // 文件名
  return `${wf}.yml`; // 自动补后缀
}

// 首次带 inputs；若工作流未声明这些 inputs 会 422，再降级为仅 runId；仍失败则再降级为“无 inputs”
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      o = await orchestrate(input);
      await writeText(runId, '01_orchestrator_mode.txt', 'online');

      // ✅ 新增：把 LLM 原始请求/响应/原文落盘（若有）
      if (o && o._trace) {
        // 最保险：把完整 trace 写一份
        await writeJSON(runId, '01a_llm_trace.json', o._trace);

        // 拆分写（若 trace 含结构化 request/response）
        if (o._trace.request) {
          await writeJSON(runId, '01a_llm_request.json', o._trace.request);
        }
        if (o._trace.response) {
          await writeJSON(runId, '01b_llm_response.json', o._trace.response);
        }

        // 原始文本（不同封装命名可能不同，做 best-effort 兜底）
        const rawText =
          o._trace.rawText ??
          o._trace.text ??
...
`;
    ...
    return NextResponse.json({
      ...
      actionsUrl,
      degraded: dispatch?.degraded ?? null, // true=降级触发；null=跳过
    });
  } catch (e: any) {
    return NextResponse.json(
