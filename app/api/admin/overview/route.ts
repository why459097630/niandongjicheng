...
export async function GET() {
  try {
    ...
    const appCloudServiceRole = process.env.APP_CLOUD_SUPABASE_SERVICE_ROLE_KEY;
    ...
    if (appCloudUrl && appCloudServiceRole) {
      ...
      if (storesResult.error) {
        console.error("NDJC admin overview: failed to load stores", storesResult.error);
      } else {
    ...
    const tabs: Record<string, TabData> = {
      dashboard: {
        metrics: [
          { title: "总用户数", value: formatCount(totalUsers), hint: "profiles" },
        ...
        notes: [
          "这一页已接 profiles / builds / stores 三个现有数据源。",
          "收入、支付、续费订单目前项目里没有正式表，先不接。",
        ],
      },
      builds: {
        metrics: [
      ...
      users: {
        metrics: [
          { title: "注册用户", value: formatCount(totalUsers), hint: "profiles" },
        ...
        notes: ["LTV、订单金额、付费价值分层当前项目没有订单表，先不接。"],
      },
      revenue: {
        metrics: [],
        notes: [
