...
const FREE_DAILY_GENERATION_LIMIT_MESSAGE =
  "You have reached today's free generation limit. Please try again tomorrow or choose Pro for more generations.";

function getFreeDailyLimitStartIso(): string {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAdmin) {
      const freeDailyLimitStart = getFreeDailyLimitStartIso();

      const { count: recentFreeBuildCount, error: recentFreeBuildError } = await supabase
        .from('builds')
        .select('id', { count: 'exact', head: true })
        .eq('user_id', user.id)
        .eq('plan', 'free')
        .gte('created_at', freeDailyLimitStart);

      if (recentFreeBuildError) {
        return NextResponse.json(
          {
            ok: false,
            error: recentFreeBuildError.message,
          },
          { status: 500 },
        );
      }

      if ((recentFreeBuildCount || 0) >= FREE_DAILY_GENERATION_LIMIT) {
        return NextResponse.json(
          {
            ok: false,
            error: FREE_DAILY_GENERATION_LIMIT_MESSAGE,
          },
          { status: 429 },
        );
      }
