...
export async function POST(req: Request) {
  try {
    ...
    // 取 HEAD SHA（供前端 run-by-sha 使用）
    const headResp = await fetch(`https://api.github.com/repos/${owner}/${repo}/commits/${ref}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json' },
    });
    if (!headResp.ok) {
      const text = await headResp.text();
      return Response.json({ ok: false, error: 'HEAD_LOOKUP_FAILED', status: headResp.status, detail: text.slice(0, 400) }, { status: 500 });
    }
    ...
    if (dispatch.status !== 204) {
      ...
      return Response.json({ ok: false, error: 'DISPATCH_FAILED', status: dispatch.status, detail: text.slice(0, 400) }, { status: 500 });
    }

    return Response.json({ ok: true, commitSha: headSha }, { headers: { 'Access-Control-Allow-Origin': '*' } });
  } catch (e: any) {
