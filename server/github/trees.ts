...
function* walk(dir: string, base = dir): Generator<{path: string, content: string}> {
  for (const name of fs.readdirSync(dir)) {
    ...
    else yield { path: rel.replaceAll("\\", "/"), content: fs.readFileSync(abs, "utf8") };
  }
}

export async function upsertRunBranchWithTree(runId: string, repoRoot: string) {
  const baseRef = await gh.rest.git.getRef({ owner: cfg.owner, repo: cfg.repo, ref: `heads/${cfg.baseRef}` });
  const baseSha = baseRef.data.object.sha;

  // 1) 创建运行分支
  const refName = `${cfg.branchPrefix}/${runId}`;
  try {
    await gh.rest.git.createRef({ owner: cfg.owner, repo: cfg.repo, ref: `refs/heads/${refName}`, sha: baseSha });
  } catch (_) { /* 已存在则继续 */ }

  // 2) 组装 tree（基于 base_tree）
  const baseCommit = await gh.rest.repos.getCommit({ owner: cfg.owner, repo: cfg.repo, ref: cfg.baseRef });
  const blobs = [...walk(path.join(repoRoot, "app")), ...walk(path.join(repoRoot, "requests"))];
