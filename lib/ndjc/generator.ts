...
async function downloadFile(url: string, dest: string) {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    h.get(url, (res) => {
      ...
      const ws = fh.createWriteStream(dest);
      res.pipe(ws);

      // ✅ 修复点：close() 返回 void，不能 then
      ws.on("finish", () => {
        ws.close();
        resolve();
      });
...
async function writeLocalesConfig(appRoot: string, localesList?: string) {
  ...
  const xml = `<locale-config xmlns:android="http://schemas.android.com/apk/res/android">
...
export function buildPlan(o: NdjcOrchestratorOutput): Patch[] {
  ...
  return [
    ...
    // themes 覆盖块
    {
      ...
      replace: [
        {
          marker: "NDJC:BLOCK:THEME_OVERRIDES",
          value: o.themeOverridesXml ?? "",
        },
      ],
    },
...
/* ========= 应用计划 ========= */
export async function applyPlanDetailed(
  plan: Patch[]
): Promise<ApplyResult[]> {
...
export async function materializeToWorkspace(
...
export async function stabilizeGradle(appRoot: string) {
  ...
  if (txt !== before) await fs.writeFile(gradleFile, "utf8");
}
...
export async function generateAndSync(
  ...
  if (replacedCount === 0) {
    throw new Error(
      "[NDJC] No critical anchors replaced (0) — abort to prevent empty APK."
    );
  }
