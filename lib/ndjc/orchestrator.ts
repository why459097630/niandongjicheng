...
async function loadRegistry(): Promise<Registry> {
  const hint = process.env.NDJC_REGISTRY_FILE || "lib/ndjc/anchors/registry.circle-basic.json";
...
async function loadRules(): Promise<Rules> {
  const hint = process.env.NDJC_RULES_FILE || "lib/ndjc/rules/ndjc-rules.json";
  try {
    const raw = await readText(hint);
...
async function loadSystemPrompt() {
  ...
  try {
    ...
    try {
      ...
      if (maybe?.system) text = String(maybe.system);
    } catch {}
...
function normalizeKey(k: string) {
  ...
  if (/^HOME_|^ROUTE_|^NAV_|^SPLASH_|^EMPTY_|^ERROR_|^DEPENDENCY_|^DEBUG_|^BUILD_|^HEADER_|^PROFILE_|^SETTINGS_/i.test(s))
    return `BLOCK:${s}`;
...
function applyWhitelist(raw: any, reg: Registry) {
  ...
  const put = (dict: any, key: string, val: any) => {
    if (key.startsWith("TEXT:")) dict.text[key.replace(/^TEXT:/, "")] = String(val ?? "");
      ...
      dict.list[key.replace(/^LIST:/, "")] = Array.isArray(val) ? val.map(String) : val == null ? [] : [String(val)];
...
function validateByRules(
    ...
    for (const k of need) {
      ...
      if (g === "list") {
        if (!Array.isArray(v) || v.length === 0) {
          doc[g][k] = placeholderFor("list", k, reg);
          report.missing.push(`${g}:${k}`);
        }
      } else if (g === "if") {
        if (typeof v !== "boolean") {
          doc[g][k] = false;
        ...
        if (v == null || String(v) === "") {
          doc[g][k] = placeholderFor(g, k, reg);
          report.missing.push(`${g}:${k}`);
        }
      }
    }
  ...
  const reject = (s: string) => (forbidAngles && /[<>]/.test(s)) || (forbidPH && s.includes(PH_TEXT));
  ...
  const checkTBH = (g: "text" | "block" | "hook") => {
    ...
    for (const [k, v] of Object.entries<string>(doc[g] || {})) {
      let s = String(v ?? "");
      if (!s.length || reject(s)) {
        doc[g][k] = placeholderFor(g, k, reg);
        report.invalid.push(`${g}:${k}`);
        continue;
      }
      const rule = fmt[k];
      if (rule?.minLen && s.length < rule.minLen) {
        s = s.padEnd(rule.minLen, ".");
        doc[g][k] = s;
        report.fixed.push(`${g}:${k}(minLen)`);
      }
      if (rule?.regex && !(new RegExp(rule.regex)).test(s)) {
        report.invalid.push(`${g}:${k}(regex)`);
      }
    }
  };
  checkTBH("text");
  ...
  for (const [k, arr] of Object.entries<any[]>(doc.list || {})) {
    ...
    let a: string[] = Array.isArray(arr) ? arr.map(String).filter(Boolean) : [];
    if (!a.length && rule?.minItems) a = Array.from({ length: rule.minItems }, () => PH_TEXT);
    if (rule?.itemRegex) {
      const re = new RegExp(rule.itemRegex);
      if (!a.every((x: string) => re.test(x))) {
        report.invalid.push(`list:${k}(itemRegex)`);
      }
    }
    doc.list[k] = a;
  }
  ...
  // applicationId
  {
    ...
    if (!PKG_REGEX.test(appId)) {
      appId = seed.applicationId;
      report.fixed.push("gradle:applicationId(fallback)");
    }
    ...
    if (!PKG_REGEX.test(String(doc.text?.["NDJC:PACKAGE_NAME"]))) {
      ...
      report.fixed.push("text:NDJC:PACKAGE_NAME(sync)");
    }
  }
  // resConfigs
  {
...
export async function orchestrate(input: OrchestrateInput): Promise<OrchestrateOutput> {
  ...
  // 2) 调 LLM（默认仅 1 次；可用 NDJC_MAX_RETRIES 调整）
  const maxRetries = Number(process.env.NDJC_MAX_RETRIES ?? "0");
  let parsed: any = null;
  let last = "";
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      _trace.retries.push({ attempt, ok: true, report });

      // 成功解析一次就结束，不再继续重试
      break;
    } catch (e: any) {
      const msg = e?.message || String(e);
      _trace.retries.push({ attempt, error: msg });
      // 命中限流/外部错误，无意义继续轰炸，直接退出循环
      if (/429|rate limit/i.test(msg)) break;
    }
  }

  // 如果完全没有解析到 LLM 返回（比如 429 之前就失败），用 skeleton 兜底
  if (!parsed) {
    parsed = { metadata: skeleton.metadata, anchors: skeleton.anchors, files: [] };
  }

  // 4) 输出及衍生
  const anchors = parsed?.anchors || skeleton.anchors;
  // 最终一致性：applicationId ↔ NDJC:PACKAGE_NAME
  const appId = ensurePackageId(
    anchors?.gradle?.applicationId || anchors?.text?.["NDJC:PACKAGE_NAME"],
    packageId
  );
