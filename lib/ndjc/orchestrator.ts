...
function buildSkeletonFromRegistry(
  ...
  for (const k of reg.text) {
    if (k === "NDJC:APP_LABEL") text[k] = seed.appName || defText[k] || "";
      ...
      text[k] = seed.packageId || defText[k] || "";
...
function buildSystemPromptFromRegistry(reg: Registry): string {
  ...
  const lines: string[] = [];
  lines.push(
    `你是“念动即成 NDJC”的模板生成助手。**只输出严格 JSON**（不要 Markdown 代码块）。`
  );
  ...
  if (reg.resources?.length)
    lines.push(`- Resources: ${reg.resources.join(", ")}`);
  ...
  if (required.block.length)
    lines.push(`- block.required: ${required.block.join(", ")}`);
  if (required.if.length)
    lines.push(`- if.required: ${required.if.join(", ")}`);
  if (required.hook.length)
    lines.push(`- hook.required: ${required.hook.join(", ")}`);
  ...
  lines.push(
    `输出必须为 **Contract v1** JSON，且键集合与“回填骨架(SKELETON)”完全一致。`
  );
  return lines.join("\n");
}
...
function normalizeKeyWithAliases(
  ...
  // 宽松推断（兜底；主要靠 registry+表单锁死）
  if (
    /^ROUTES$|FIELDS$|FLAGS$|STYLES$|PATTERNS$|PROGUARD|SPLITS|STRINGS$/i.test(k)
  )
    return `LIST:${k}`;
...
function normalizeAnchorsUsingRegistry(raw: any, reg: Registry) {
  ...
  for (const g of groups) {
    ...
    for (const [k, v] of Object.entries(m)) {
      const key1 = /^(TEXT|BLOCK|LIST|IF|HOOK):/.test(k)
        ? k
...
export async function orchestrate(input: OrchestrateInput): Promise<OrchestrateOutput> {
  ...
  let _trace: any | null = { retries: [] as any[], rawText: "" };
    ...
    try {
      ...
      if (attempt > 0 && lastText) {
        ...
        msgs.push({
          role: "user",
          content:
            "上面的内容不合规。请根据反馈修正并重新返回完整 JSON（同样的键集合）。",
        });
      }

      const r = await callGroqChat(msgs, { json: true, temperature: 0 });

      // —— 关键修复：无论返回类型是什么，都构造“原始可校验字符串” —— //
      const rawCandidate =
        typeof r === "string"
          ? r
          : (r as any)?.text != null
          ...
          : JSON.stringify(r ?? {});
      _trace.llm = { messages: msgs, response: r };
      _trace.rawText = rawCandidate; // 提供给 contract-precheck
      lastText = rawCandidate;

      const maybe = parseJsonSafely(rawCandidate) as any;
      ...
      // 即使异常也要保证 rawText 存在（给预检用）
      if (!_trace.rawText) _trace.rawText = lastText || "{}";
    }
  }
  ...
  // 兜底：若必须 v1 且完全无返回，则合成最小 v1，并写入 rawText
  if (wantV1(input) && (!parsed || !parsed._text)) {
    ...
    const synthesized = JSON.stringify(v1doc);
    _trace.synthesized = true;
    _trace.rawText = synthesized;
  } else {
    // 确保 rawText 一定有值
    if (!_trace.rawText) {
      _trace.rawText =
        (parsed && typeof parsed._text === "string" && parsed._text) ||
        JSON.stringify(parsed?._raw ?? {});
    }
